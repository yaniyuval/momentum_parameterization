...
C
     ...
     *     FV,  FTRA, PRECIP, WD,   TPRIME, QPRIME, CBMF, CU, 
     *     DAMP, DTMAX, MINORIG, TOT_MF)
C
C-----------------------------------------------------------------------------
C    *** On input:      ***
C
...
C
      ...
      REAL CU, DAMP, DTMAX, MINORIG
      REAL CBMF(1), PRECIP(1), TOT_MF(ND)
Cf2py intent(inplace) FT
Cf2py intent(inplace) FQ
Cf2py intent(inplace) FU
Cf2py intent(inplace) FV
Cf2py intent(inplace) PRECIP
Cf2py intent(inplace) CBMF
...
C
     ...
     1      CL*Q(J)+QS(J)*(CPV-CL+ALV*ALV/(RV*T(J)*T(J))))
           ...
           PRECIP(1)=PRECIP(1)+24.*3600.*1.0E5*(PH(J)-PH(J+1))*
...
C
        IF(CBMF(1).EQ.0.0.AND.TVP(ICB).LE.(TV(ICB)-DTMAX))THEN
         IFLAG=0
         RETURN
        END IF
C
...
C
      CBMFOLD=CBMF(1)
	DELT0=300.0
C **** JY       DAMPS=DAMP*DELT/DELT0
C **** JY       CBMF(1)=0.1*ALPHA*DTMA/DAMPS 
C      print *, "CBMF = ", CBMF(1)
C      *** CBMF=(1.-DAMPS)*CBMF+0.1*ALPHA*DTMA *** JY Commented
C*** JY: DAMP HERE IS MULTIPLYING THE INPUT CBMF SO I COULD TUNE 
C      CBMF(1)=DAMP * MAX(CBMF(1),0.0)
      CBMF(1)=DAMP * MAX(TOT_MF(7),0.0) 
C
C   *** If cloud base mass flux is zero, skip rest of calculation  ***
C
      IF(CBMF(1).EQ.0.0.AND.CBMFOLD.EQ.0.0)THEN
       RETURN
      END IF
C
...
C *** JY : Prescribe Mass flux at all levels:
C      DO 1999 I=ICB+1,INB
C       M(I) = TOT_MF(I)
C      1999 CONTINUE
...
C
        PRECIP(1)=PRECIP(1)+WT(1)*SIGD*WATER(1)*3600.*24000./(ROWL*G)
C
  405   CONTINUE
C
