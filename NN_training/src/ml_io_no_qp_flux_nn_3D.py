...
def build_training_dataset(expt, start_time, end_time, interval, n_x_samp=5, train_size=0.9, do_shuffle=True,
    ...
    if flag_dict['sst_in']: # For the surface fluxes.
        ...
        elif flag_dict['resolution'] == 4:
            solin_file = '/glade/scratch/janniy/ML_convection_data/solin/sst_x4.nc'
            f_sst = Dataset(solin_file, mode='r')
            sst = f_sst.variables['sst_x4'][:]  # m
        else: # Did not create yet fields for other SOLIN
            raise Exception('SST: So far got the SST only for x16 and x8 cases and x4 cases.')
    ...
    if 'qsat_feature' in flag_dict.keys():
        if flag_dict['qsat_feature']:
            qsat = np.zeros((n_z_input, n_y, n_x_samp, n_files))

    if 'RH_not_q' in flag_dict.keys():
        if flag_dict['RH_not_q']:
            RHin= np.zeros((n_z_input, n_y, n_x_samp, n_files))
    ...
    for ifile, file_time in enumerate(file_times):
        ...
        if 'qsat_feature' in flag_dict.keys():
            if flag_dict['qsat_feature']:
                zqsat = np.zeros((n_z, n_y, n_x))
                zqsat_try = np.zeros((n_z, n_y, n_x))

        if 'RH_not_q' in flag_dict.keys():
            if flag_dict['RH_not_q']:
                zRHin = np.zeros((n_z, n_y, n_x))
        ...
        if flag_dict['do_hor_wind_input']:
            ...
            # Defining wind at the grid of q,Tx
            for i in range(n_x):
                if i == n_x - 1:
                    indx1 = i
                    indx2 = 0
        ...
        if flag_dict['calc_tkz_z']:
            if flag_dict['calc_tkz_z_correction']:
                tkh_z[:, :, :] = tkz_z[:, :, :]  # JY - added Feb 2021 for momentum
                #tkh_z[:, :, :] = tkz_z[:, :, :] * Pr1[:, :, :] - tkz_z_res[:, :, :] * Pr1_res[:, :, :]
            else:
                tkh_z[:, :, :] = tkz_z[:, :, :] * Pr1[:, :, :]

        # Trying to use qsat as a feature: (I verified that it is like doing element by element calculation)
        if 'qsat_feature' in flag_dict.keys():
            if flag_dict['qsat_feature']:
                a_bg = 1. / (atmos_physics.tbgmax - atmos_physics.tbgmin)
                omn = np.maximum(0.0, np.minimum(1.0, (tabs - atmos_physics.tbgmin) * a_bg))
                for k in range(n_z):
                    zqsat[k,:,:] = omn[k, :, :] * atmos_physics.sam_qsatw(tabs[k, :, :], p[k])\
                                + (1. - omn[k, :, :]) * atmos_physics.sam_qsati(tabs[k, :, :],p[k])
        ...
        for j in range(zTin.shape[1]):
            ...
            if 'qsat_feature' in flag_dict.keys():
                if flag_dict['qsat_feature']:
                    qsat[:, j, :, ifile] = zqsat[0:n_z_input, j, :][:, ind_x]

            if 'RH_not_q' in flag_dict.keys():
                if flag_dict['RH_not_q']:
                    RHin[:, j, :, ifile] = zRHin[0:n_z_input, j, :][:, ind_x]
    ...
    if 'qsat_feature' in flag_dict.keys():
        if flag_dict['qsat_feature']:
            train_input_list.append(np.float32(qsat[:, :,:, randind_trn]))
            test_input_list.append(np.float32(qsat[:, :,:, randind_tst]))
    if 'RH_not_q' in flag_dict.keys():
        if flag_dict['RH_not_q']:
            train_input_list.append(np.float32(RHin[:, :, :, randind_trn]))
            test_input_list.append(np.float32(RHin[:, :, :, randind_tst]))
    if flag_dict['do_surf_wind']:
    ...
    pickle.dump(train_input_list,open(output_dir + expt + data_specific_description + '_training_3D.pkl', 'wb'))
    pickle.dump(test_input_list,open(output_dir + expt + data_specific_description + '_testing_3D.pkl', 'wb'))
...
def create_specific_data_string_desc(flag_dict):
    ...
    if 'RH_not_q' in flag_dict.keys():
        if flag_dict['RH_not_q']:
            data_specific_description = data_specific_description + '_RH'
    if 'qsat_feature' in flag_dict.keys():
        if flag_dict['qsat_feature']:
            print('using qsat as feature')
            data_specific_description = data_specific_description + '_qsat'
